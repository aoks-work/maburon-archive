...
const MonthlyPage = ({ data  }) => {
  ...
  <Layout>
    ...
    <h3 className={link_styles.pageMonth}>{data.yearOf2019Json.month}月</h3>
    <div className={link_styles.podcast_link}>  {/* podcast playlist bubka */}
        <iframe
        src={data.yearOf2019Json.podcastUrl}
        title="podcast"
        ></iframe>
    </div>
    <div className={link_styles.playlist_link}>
        <iframe
        src={data.yearOf2019Json.playlistUrl}
        title="podcast"
        ></iframe>
    </div> {/* podcast playlist bubka */}

    <div className="json-data"> {/* 楽曲リスト */}
        {data.yearOf2019Json.track_list.map((list) => (
