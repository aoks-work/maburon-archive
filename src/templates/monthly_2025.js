...
const MonthlyPage = ({ data  }) => {
  ...
  <Layout>
    ...
    <h3 className={link_styles.pageMonth}>{data.yearOf2025Json.month}月</h3>
    <div className={link_styles.podcast_link}>  {/* podcast playlist bubka */}
        <iframe
        src={data.yearOf2025Json.podcastUrl}
        title="podcast"
        ></iframe>
    </div>
    <div className={link_styles.playlist_link}>
        <iframe
        src={data.yearOf2025Json.playlistUrl}
        title="podcast"
        ></iframe>
    </div>
    <div className={link_styles.bubka_link}>
    {data.yearOf2025Json.bubkaUrl !== "" &&
        <p>宇多丸さんのコラムが掲載している雑誌「BUBKA」では連載コラムのみをまとめたコラムパックがお得に販売されています。マブ論のコラムが読みたい方は
        <a className={link_styles.column_link} href={data.yearOf2025Json.bubkaUrl} target="_blank" rel="noopener noreferrer">こちら</a>
        がオススメ！！</p>
    }
    </div> {/* podcast playlist bubka */}

    <div className="json-data"> {/* 楽曲リスト */}
        {data.yearOf2025Json.track_list.map((list) => (
